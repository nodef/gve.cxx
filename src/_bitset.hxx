...
class LazyBitset {
  ...
  inline auto find(K k) const noexcept {
    ...
    auto   it = std::lower_bound(begin(), end(), k, fl);
 ...
 */
template <class B>
void writeBitset(std::ostream& a, const B& x) {
  a << "{\n";
...
inline void write(std::ostream& a, const LazyBitset<K, V>& x) {
  writeBitset(a, x);
}
 ...
 */
template <class K, class V>
inline std::ostream& operator<<(std::ostream& a, const LazyBitset<K, V>& x) {
  write(a, x);
  return a;
}
#pragma endregion
