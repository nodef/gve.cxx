 ...
 * @returns number of lines
 */
inline size_t countLines(const std::string& x) {
...
inline I parseFloatW(T &a, I ib, I ie) {
  ...
  a = (T(u) + T(v) * std::pow(10, -d)) * T(std::pow(10, e));
  if (neg) a = -a;
  return ib;
}
...
inline I parseNumberW(T& a, I ib, I ie) {
  IF_CONSTEXPR      (std::is_integral<T>::value)       return parseIntegerW<FULL>(a, ib, ie);
  else IF_CONSTEXPR (std::is_floating_point<T>::value) return parseFloatW<FULL>  (a, ib, ie);
  return ib;
}
#pragma endregion
...
inline I readTokenW(std::string_view& a, I ib, I ie, FU fu, FW fw) {
  auto [tb, te] = findNextToken(ib, ie, fu, fw);
  IF_CONSTEXPR (CHECK) { if (tb==te) throw FormatError("Failed to read token (empty)", tb); }
  a  = std::string_view(&*tb, te-tb);
  return te;
}
...
inline I readNumberW(T& a, I ib, I ie, FU fu, FW fw) {
  auto [tb, te] = findNextToken(ib, ie, fu, fw);
  IF_CONSTEXPR (CHECK) { if (tb==te) throw FormatError("Failed to read number (empty)", tb); }
  tb = parseNumberW<CHECK? 2:1>(a, tb, te);
  IF_CONSTEXPR (CHECK) { if (tb!=te) throw FormatError("Failed to read number (bad format)", tb); }
  return te;
}
#pragma endregion
#pragma endregion
