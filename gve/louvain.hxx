...
struct LouvainResult {
   ...
   */
  LouvainResult(vector<K>& membership, vector<W>& vertexWeight, vector<W>& communityWeight, int iterations=0, int passes=0, float time=0, float markingTime=0, float initializationTime=0, float firstPassTime=0, float localMoveTime=0, float aggregationTime=0, size_t affectedVertices=0) :
  membership(std::move(membership)), vertexWeight(std::move(vertexWeight)), communityWeight(std::move(communityWeight)), iterations(iterations), passes(passes), time(time), markingTime(markingTime), initializationTime(initializationTime), firstPassTime(firstPassTime), localMoveTime(localMoveTime), aggregationTime(aggregationTime), affectedVertices(affectedVertices) {}
  #pragma endregion
};
#pragma endregion
...
inline auto louvainChooseCommunity(const G& x, K u, const vector<K>& vcom, const vector<W>& vtot, const vector<W>& ctot, const vector<K>& vcs, const vector<W>& vcout, double M, double R) {
  ...
  return std::make_pair(cmax, emax);
}
...
EXPORT namespace gve {
  // Types
  using detail::LouvainOptions;
  using detail::LouvainResult;
  // Methods
  // using detail::louvainAllocateHashtablesW;
  // using detail::louvainFreeHashtablesW;
  // using detail::louvainVertexWeightsW;
  // using detail::louvainCommunityWeightsW;
  // using detail::louvainInitializeW;
  // using detail::louvainInitializeFromW;
  // using detail::louvainUpdateWeightsFromU;
  // using detail::louvainScanCommunityW;
  // using detail::louvainScanCommunitiesW;
  // using detail::louvainClearScanW;
  // using detail::louvainChooseCommunity;
  // using detail::louvainChangeCommunityW;
  // using detail::louvainMoveW;
  // using detail::louvainCommunityExistsW;
  // using detail::louvainCommunityTotalDegreeW;
  // using detail::louvainCountCommunityVerticesW;
  // using detail::louvainCommunityVerticesW;
  // using detail::louvainLookupCommunitiesU;
  // using detail::louvainAggregateEdgesW;
  // using detail::louvainRenumberCommunitiesW;
  // using detail::louvainAggregateW;
  // using detail::louvainInvoke;
  // using detail::louvainSetupInitialsW;
  using detail::louvainStatic;
#ifdef _OPENMP
  // using detail::louvainVertexWeightsOmpW;
  // using detail::louvainCommunityWeightsOmpW;
  // using detail::louvainInitializeOmpW;
  // using detail::louvainInitializeFromOmpW;
  // using detail::louvainUpdateWeightsFromOmpU;
  // using detail::louvainChangeCommunityOmpW;
  // using detail::louvainMoveOmpW;
  // using detail::louvainCommunityExistsOmpW;
  // using detail::louvainCommunityTotalDegreeOmpW;
  // using detail::louvainCountCommunityVerticesOmpW;
  // using detail::louvainCommunityVerticesOmpW;
  // using detail::louvainLookupCommunitiesOmpU;
  // using detail::louvainAggregateEdgesOmpW;
  // using detail::louvainRenumberCommunitiesOmpW;
  // using detail::louvainAggregateOmpW;
  // using detail::louvainInvokeOmp;
  using detail::louvainStaticOmp;
#endif
} // namespace gve
